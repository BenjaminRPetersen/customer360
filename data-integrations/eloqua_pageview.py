# -*- coding: utf-8 -*-
"""
Title: Import Page View

Created on Mon Sep 13 14:20:20 2021
...
fields = fields[fields.activityTypes.apply(lambda x: 'PageView' in x)].reset_index()
...
payload = '''{
              "name":"BI Fusion PageView Export",
              "fields":{
                    '''+f+'''
                    }
              "filter":"'{{Activity.Type}}' = 'PageView' AND '{{Activity.CreatedAt}}' >= \''''+yesterday+''' 00:00:00.000'"
...
"""
conn.execute("CREATE TABLE customer360.t_eloqua_pageview AS (SELECT * FROM customer360.s_eloqua_pageview WHERE 1=2);")
activities.to_sql('t_eloqua_pageview', conn, schema='customer360', index=False, if_exists='append',chunksize=10000, method='multi')
...
created = conn.execute("select count(*) ct from customer360.t_eloqua_pageview where id not in (select id from customer360.s_eloqua_pageview)").fetchall()[0][0]
updated = conn.execute("select count(*) ct from customer360.t_eloqua_pageview where id in (select id from customer360.s_eloqua_pageview)").fetchall()[0][0]
conn.execute("   update customer360.s_eloqua_pageview s\
                 set    "+update+" \
                from    customer360.t_eloqua_pageview t\
                where   s.id = t.id; commit;")
conn.execute("insert into customer360.s_eloqua_pageview select * from customer360.t_eloqua_pageview where id not in (select id from customer360.s_eloqua_pageview); commit;") # Insert New Records
conn.execute("drop table customer360.t_eloqua_pageview; commit;")
