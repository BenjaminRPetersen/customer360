# -*- coding: utf-8 -*-
"""
Title: Salesforce Event Integration

Created on Wed Sep 15 15:58:01 2021
...
"""
conn.execute("CREATE TABLE customer360.t_salesforce_event AS (SELECT * FROM customer360.s_salesforce_event WHERE 1=2);")
df.to_sql('t_salesforce_event', conn, schema='customer360', index=False, if_exists='append',chunksize=10000, method='multi')
...
updated = conn.execute("select count(*) ct from customer360.t_salesforce_event where id in (select id from customer360.s_salesforce_event)").fetchall()[0][0]
created = conn.execute("select count(*) ct from customer360.t_salesforce_event where id not in (select id from customer360.s_salesforce_event)").fetchall()[0][0]
conn.execute("   update customer360.s_salesforce_event s\
                 set    "+update+" \
                from    customer360.t_salesforce_event t\
                where   s.id = t.id; commit;")
conn.execute("insert into customer360.s_salesforce_event select * from customer360.t_salesforce_event where id not in (select id from customer360.s_salesforce_event); commit;") # Insert New Records
conn.execute("drop table customer360.t_salesforce_event; commit;")
