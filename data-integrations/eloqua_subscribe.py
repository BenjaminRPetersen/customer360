# -*- coding: utf-8 -*-
"""
Title: Import Eloqua Subscription Data

Created on Mon Sep 13 14:25:07 2021
...
fields = fields[fields.activityTypes.apply(lambda x: 'Subscribe' in x)].reset_index()
...
payload = '''{
              "name":"BI Fusion Subscribe Export",
              "fields":{
                    '''+f+'''
                    }
              "filter":"'{{Activity.Type}}' = 'Subscribe' AND '{{Activity.CreatedAt}}' >= \''''+yesterday+''' 00:00:00.000'"
...
"""
conn.execute("CREATE TABLE customer360.t_eloqua_subscribe AS (SELECT * FROM customer360.s_eloqua_subscribe WHERE 1=2);")
activities.to_sql('t_eloqua_subscribe', conn, schema='customer360', index=False, if_exists='append',chunksize=10000, method='multi')
...
created = conn.execute("select count(*) ct from customer360.t_eloqua_subscribe where id not in (select id from customer360.s_eloqua_subscribe)").fetchall()[0][0]
updated = conn.execute("select count(*) ct from customer360.t_eloqua_subscribe where id in (select id from customer360.s_eloqua_subscribe)").fetchall()[0][0]
conn.execute("   update customer360.s_eloqua_subscribe s\
                 set    "+update+" \
                from    customer360.t_eloqua_subscribe t\
                where   s.id = t.id; commit;")
conn.execute("insert into customer360.s_eloqua_subscribe select * from customer360.t_eloqua_subscribe where id not in (select id from customer360.s_eloqua_subscribe); commit;") # Insert New Records
conn.execute("drop table customer360.t_eloqua_subscribe; commit;")
