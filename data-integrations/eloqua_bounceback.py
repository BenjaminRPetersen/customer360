# -*- coding: utf-8 -*-
"""
Title: Import Eloqua Bounceback Data

Created on Mon Sep 13 14:37:32 2021
...
fields = fields[fields.activityTypes.apply(lambda x: 'Bounceback' in x)].reset_index()
...
payload = '''{
              "name":"BI Fusion Bounceback Export",
              "fields":{
                    '''+f+'''
                    }
              "filter":"'{{Activity.Type}}' = 'Bounceback' AND '{{Activity.CreatedAt}}' >= \''''+yesterday+''' 00:00:00.000'"
...
"""
conn.execute("CREATE TABLE customer360.t_eloqua_bounceback AS (SELECT * FROM customer360.s_eloqua_bounceback WHERE 1=2);")
activities.to_sql('t_eloqua_bounceback', conn, schema='customer360', index=False, if_exists='append',chunksize=10000, method='multi')
...
created = conn.execute("select count(*) ct from customer360.t_eloqua_bounceback where id not in (select id from customer360.s_eloqua_bounceback)").fetchall()[0][0]
updated = conn.execute("select count(*) ct from customer360.t_eloqua_bounceback where id in (select id from customer360.s_eloqua_bounceback)").fetchall()[0][0]   
conn.execute("   update customer360.s_eloqua_bounceback s\
                 set    "+update+" \
                from    customer360.t_eloqua_bounceback t\
                where   s.id = t.id; commit;")
conn.execute("insert into customer360.s_eloqua_bounceback select * from customer360.t_eloqua_bounceback where id not in (select id from customer360.s_eloqua_bounceback); commit;") # Insert New Records
conn.execute("drop table customer360.t_eloqua_bounceback; commit;")
