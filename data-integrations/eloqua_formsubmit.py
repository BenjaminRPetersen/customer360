# -*- coding: utf-8 -*-
"""
Title: Import Form Submission data from Eloqua

Created on Mon Sep 13 13:16:43 2021
...
fields = fields[fields.activityTypes.apply(lambda x: 'FormSubmit' in x)].reset_index()
...
payload = '''{
              "name":"BI Fusion FormSubmit Export",
              "fields":{
                    '''+f+'''
                    }
              "filter":"'{{Activity.Type}}' = 'FormSubmit' AND '{{Activity.CreatedAt}}' >= \''''+yesterday+''' 00:00:00.000'"
...
"""
conn.execute("CREATE TABLE customer360.t_eloqua_formsubmit AS (SELECT * FROM customer360.s_eloqua_formsubmit WHERE 1=2);")
activities.to_sql('t_eloqua_formsubmit', conn, schema='customer360', index=False, if_exists='append',chunksize=10000, method='multi')
...
created = conn.execute("select count(*) ct from customer360.t_eloqua_formsubmit where id not in (select id from customer360.s_eloqua_formsubmit)").fetchall()[0][0]
updated = conn.execute("select count(*) ct from customer360.t_eloqua_formsubmit where id in (select id from customer360.s_eloqua_formsubmit)").fetchall()[0][0]
conn.execute("   update customer360.s_eloqua_formsubmit s\
                 set    "+update+" \
                from    customer360.t_eloqua_formsubmit t\
                where   s.id = t.id; commit;")
conn.execute("insert into customer360.s_eloqua_formsubmit select * from customer360.t_eloqua_formsubmit where id not in (select id from customer360.s_eloqua_formsubmit); commit;") # Insert New Records
conn.execute("drop table customer360.t_eloqua_formsubmit; commit;")
