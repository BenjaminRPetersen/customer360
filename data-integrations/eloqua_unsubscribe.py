# -*- coding: utf-8 -*-
"""
Title: Import Eloqua Unsubscribe Data

Created on Mon Sep 13 14:31:04 2021
...
fields = fields[fields.activityTypes.apply(lambda x: 'Unsubscribe' in x)].reset_index()
...
payload = '''{
              "name":"BI Fusion Unsubscribe Export",
              "fields":{
                    '''+f+'''
                    }
              "filter":"'{{Activity.Type}}' = 'Unsubscribe' AND '{{Activity.CreatedAt}}' >= \''''+yesterday+''' 00:00:00.000'"
...
"""
conn.execute("CREATE TABLE customer360.t_eloqua_unsubscribe AS (SELECT * FROM customer360.s_eloqua_unsubscribe WHERE 1=2);")
activities.to_sql('t_eloqua_unsubscribe', conn, schema='customer360', index=False, if_exists='append',chunksize=10000, method='multi')
...
conn.execute("   update customer360.s_eloqua_unsubscribe s\
                 set    "+update+" \
                from    customer360.t_eloqua_unsubscribe t\
                where   s.id = t.id; commit;")
conn.execute("insert into customer360.s_eloqua_unsubscribe select * from customer360.t_eloqua_unsubscribe where id not in (select id from customer360.s_eloqua_unsubscribe); commit;") # Insert New Records
conn.execute("drop table customer360.t_eloqua_unsubscribe; commit;")
