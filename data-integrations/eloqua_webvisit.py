# -*- coding: utf-8 -*-
"""
Title: Import Webvisits from Eloqua

Created on Mon Sep 13 12:05:01 2021
...
fields = fields[fields.activityTypes.apply(lambda x: 'WebVisit' in x)].reset_index()
...
payload = '''{
              "name":"BI Fusion Webvisit Export",
              "fields":{
                    '''+f+'''
                    }
              "filter":"'{{Activity.Type}}' = 'WebVisit' AND '{{Activity.CreatedAt}}' >= \''''+yesterday+''' 00:00:00.000'"
...
"""
conn.execute("CREATE TABLE customer360.t_eloqua_webvisit AS (SELECT * FROM customer360.s_eloqua_webvisit WHERE 1=2);")
activities.to_sql('t_eloqua_webvisit', conn, schema='customer360', index=False, if_exists='append',chunksize=10000, method='multi')
...
created = conn.execute("select count(*) ct from customer360.t_eloqua_webvisit where id not in (select id from customer360.s_eloqua_webvisit)").fetchall()[0][0]
updated = conn.execute("select count(*) ct from customer360.t_eloqua_webvisit where id in (select id from customer360.s_eloqua_webvisit)").fetchall()[0][0]
conn.execute("   update customer360.s_eloqua_webvisit s\
                 set    "+update+" \
                from    customer360.t_eloqua_webvisit t\
                where   s.id = t.id; commit;")
conn.execute("insert into customer360.s_eloqua_webvisit select * from customer360.t_eloqua_webvisit where id not in (select id from customer360.s_eloqua_webvisit); commit;") # Insert New Records
conn.execute("drop table customer360.t_eloqua_webvisit; commit;")
