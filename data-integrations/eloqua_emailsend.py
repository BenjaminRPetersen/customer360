# -*- coding: utf-8 -*-
"""
Title: Import Email Send data from Eloqua
Created on Mon Sep 13 13:12:08 2021
...
fields = fields[fields.activityTypes.apply(lambda x: 'EmailSend' in x)].reset_index()
...
payload = '''{
              "name":"BI Fusion EmailSend Export",
              "fields":{
                    '''+f+'''
                    }
              "filter":"'{{Activity.Type}}' = 'EmailSend' AND '{{Activity.CreatedAt}}' >= \''''+yesterday+''' 00:00:00.000'"
...
"""
conn.execute("CREATE TABLE customer360.t_eloqua_emailsend AS (SELECT * FROM customer360.s_eloqua_emailsend WHERE 1=2);")
activities.to_sql('t_eloqua_emailsend', conn, schema='customer360', index=False, if_exists='append',chunksize=10000, method='multi')
...
updated = conn.execute("select count(*) ct from customer360.t_eloqua_emailsend where id in (select id from customer360.s_eloqua_emailsend)").fetchall()[0][0]
conn.execute("   update customer360.s_eloqua_emailsend s\
                 set    "+update+" \
                from    customer360.t_eloqua_emailsend t\
                where   s.id = t.id;  commit;")
created = conn.execute("select count(*) ct from customer360.t_eloqua_emailsend where id not in (select id from customer360.s_eloqua_emailsend);").fetchall()[0][0]
conn.execute("insert into customer360.s_eloqua_emailsend select * from customer360.t_eloqua_emailsend where id not in (select id from customer360.s_eloqua_emailsend); commit;") # Insert New Records
conn.execute("drop table customer360.t_eloqua_emailsend; commit;")
