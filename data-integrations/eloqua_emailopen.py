# -*- coding: utf-8 -*-
"""
Title: Import Email Open data from Eloqua

Created on Mon Sep 13 12:55:54 2021
...
fields = fields[fields.activityTypes.apply(lambda x: 'EmailOpen' in x)].reset_index()
...
payload = '''{
              "name":"BI Fusion EmailOpen Export",
              "fields":{
                    '''+f+'''
                    }
              "filter":"'{{Activity.Type}}' = 'EmailOpen' AND '{{Activity.CreatedAt}}' >= \''''+yesterday+''' 00:00:00.000'"
...
"""
conn.execute("CREATE TABLE customer360.t_eloqua_emailopen AS (SELECT * FROM customer360.s_eloqua_emailopen WHERE 1=2);")
activities.to_sql('t_eloqua_emailopen', conn, schema='customer360', index=False, if_exists='append',chunksize=10000, method='multi')
...
updated = conn.execute("select count(*) ct from customer360.t_eloqua_emailopen where id in (select id from customer360.s_eloqua_emailopen)").fetchall()[0][0]
conn.execute("   update customer360.s_eloqua_emailopen s\
                 set    "+update+" \
                from    customer360.t_eloqua_emailopen t\
                where   s.id = t.id;  commit;")
created = conn.execute("select count(*) ct from customer360.t_eloqua_emailopen where id not in (select id from customer360.s_eloqua_emailopen)").fetchall()[0][0]
conn.execute("insert into customer360.s_eloqua_emailopen select * from customer360.t_eloqua_emailopen where id not in (select id from customer360.s_eloqua_emailopen);  commit;") # Insert New Records
conn.execute("drop table customer360.t_eloqua_emailopen;  commit;")
